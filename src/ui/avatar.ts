...
function bodyLayer(fill: string): string {
  ...
  <g>
    <path d="M100 34
          ...
          fill="${fill}" stroke="${B.line}" stroke-width="3"/>
    <!-- pleats gathered at the top, like a real dumpling squishy -->
    <path d="M100 34 C 97 44 95 52 94 60" fill="none" stroke="${B.lineSoft}" stroke-width="2.5" stroke-linecap="round"/>
    <path d="M100 34 C 103 44 105 52 106 60" fill="none" stroke="${B.lineSoft}" stroke-width="2.5" stroke-linecap="round"/>
    <path d="M88 37 C 84 46 81 53 79 62" fill="none" stroke="${B.lineSoft}" stroke-width="2.5" stroke-linecap="round"/>
    <path d="M112 37 C 116 46 119 53 121 62" fill="none" stroke="${B.lineSoft}" stroke-width="2.5" stroke-linecap="round"/>
    <path d="M76 43 C 71 51 67 58 64 66" fill="none" stroke="${B.lineFaint}" stroke-width="2.5" stroke-linecap="round"/>
    <path d="M124 43 C 129 51 133 58 136 66" fill="none" stroke="${B.lineFaint}" stroke-width="2.5" stroke-linecap="round"/>
    <!-- top knot -->
    <ellipse cx="100" cy="32" rx="10" ry="7" fill="${fill}" stroke="${B.line}" stroke-width="2.5"/>
    <!-- soft highlight -->
    <ellipse cx="72" cy="82" rx="22" ry="14" fill="${B.highlight}" transform="rotate(-20 72 82)"/>
    <!-- blush -->
    <ellipse cx="62" cy="122" rx="11" ry="7" fill="${B.blush}"/>
    <ellipse cx="138" cy="122" rx="11" ry="7" fill="${B.blush}"/>
  </g>`;
}
  ...
  switch (id) {
    ...
    case 'wink':
      return `<g>
        ...
        <circle cx="126.5" cy="107.5" r="2.2" fill="${B.white}"/></g>`;
    case 'hearts':
      return `<g fill="${B.heart}" stroke="${B.heartEdge}" stroke-width="1.5">
    ...
    case 'big':
      return `<g>
        <circle cx="76" cy="110" r="11" fill="${B.white}" stroke="${INK}" stroke-width="3.5"/>
        <circle cx="124" cy="110" r="11" fill="${B.white}" stroke="${INK}" stroke-width="3.5"/>
    ...
    case 'sparkly':
      return `<g>
        ...
        <circle cx="79" cy="105" r="4" fill="${B.white}"/><circle cx="127" cy="105" r="4" fill="${B.white}"/>
        <circle cx="72.5" cy="116" r="2" fill="${B.white}" opacity="0.75"/>
        <circle cx="120.5" cy="116" r="2" fill="${B.white}" opacity="0.75"/>
        <path d="M89 97 l1.7 4.3 4.3 1.7 -4.3 1.7 -1.7 4.3 -1.7-4.3 -4.3-1.7 4.3-1.7 Z" fill="${B.sparkle}"/>
        <path d="M137 97 l1.7 4.3 4.3 1.7 -4.3 1.7 -1.7 4.3 -1.7-4.3 -4.3-1.7 4.3-1.7 Z" fill="${B.sparkle}"/></g>`;
    case 'dizzy':
    ...
    case 'shekel':
      ...
      return `<g font-family="system-ui, -apple-system, Arial, sans-serif"
        ...
        <g stroke="${B.shekelHalo}" stroke-width="5" stroke-linejoin="round" fill="${B.shekelHalo}">
          <text x="76" y="122">₪</text><text x="124" y="122">₪</text></g>
        <g fill="${B.shekelFill}"><text x="76" y="122">₪</text><text x="124" y="122">₪</text></g></g>`;
    case 'dot':
    default:
      return `<g fill="${INK}">
        <circle cx="76" cy="110" r="7"/><circle cx="124" cy="110" r="7"/>
        <circle cx="78.5" cy="107.5" r="2.2" fill="${B.white}"/><circle cx="126.5" cy="107.5" r="2.2" fill="${B.white}"/></g>`;
  }
}
  ...
  switch (id) {
    case 'open':
      return `<g><ellipse cx="100" cy="136" rx="11" ry="13" fill="${INK}"/>
        <ellipse cx="100" cy="142" rx="6.5" ry="6" fill="${B.mouthInner}"/></g>`;
    case 'cat':
    ...
    case 'tongue':
      return `<g><path d="M84 130 Q 100 143 116 130" fill="none" stroke="${INK}" stroke-width="5" stroke-linecap="round"/>
        <path d="M94 136 Q 100 148 106 136 Z" fill="${B.mouthInner}" stroke="${B.tongueEdge}" stroke-width="1.5"/></g>`;
    case 'kiss':
      return `<ellipse cx="100" cy="133" rx="6" ry="7" fill="${INK}"/>`;
    case 'grin':
      return `<g><path d="M82 128 Q 100 152 118 128 Z" fill="${INK}"/>
        <path d="M92 139 Q 100 147 108 139 L 108 141 Q 100 149 92 141 Z" fill="${B.mouthInner}"/></g>`;
    case 'wavy':
    ...
    case 'ooh':
      return `<g><circle cx="100" cy="134" r="7.5" fill="${INK}"/>
        <circle cx="100" cy="136.5" r="3.5" fill="${B.mouthInner}"/></g>`;
    case 'smirk':
    ...
    case 'fangs':
      return `<g>
        ...
        <path d="M91.5 134.5 l4.5 0.8 -2.8 5.6 Z" fill="${B.white}" stroke="${INK}" stroke-width="1.5" stroke-linejoin="round"/>
        <path d="M104 135.3 l4.5 -0.8 -1.7 5.9 Z" fill="${B.white}" stroke="${INK}" stroke-width="1.5" stroke-linejoin="round"/></g>`;
    case 'drool':
      return `<g>
        ...
        <path d="M109 139 Q 113.5 147 110 152.5 Q 105.5 148 109 139 Z"
          fill="${B.drool}" stroke="${B.droolEdge}" stroke-width="1.5"/></g>`;
    case 'smile':
    default:
  ...
  switch (id) {
    case 'bow':
      return `<g transform="translate(136 46) rotate(18)">
        <path d="M0 0 L-20 -11 Q -25 0 -20 11 Z" fill="${B.heart}" stroke="${B.heartEdge}" stroke-width="2"/>
        <path d="M0 0 L20 -11 Q 25 0 20 11 Z" fill="${B.heart}" stroke="${B.heartEdge}" stroke-width="2"/>
        <circle cx="0" cy="0" r="5.5" fill="${B.bowKnot}" stroke="${B.heartEdge}" stroke-width="2"/></g>`;
    case 'cap':
      return `<g>
        <path d="M64 42 Q 100 8 136 42 L 136 52 Q 100 36 64 52 Z" fill="${B.capBlue}" stroke="${B.capBlueEdge}" stroke-width="2.5"/>
        <path d="M128 44 Q 158 44 164 56 Q 146 60 130 54 Z" fill="${B.capBlue}" stroke="${B.capBlueEdge}" stroke-width="2.5"/>
        <circle cx="100" cy="22" r="6" fill="${B.capBlueEdge}"/></g>`;
    case 'glasses':
    ...
    case 'flower':
      return `<g transform="translate(138 48) rotate(15)">
        <g fill="${B.flowerPink}" stroke="${B.flowerPinkEdge}" stroke-width="1.5">
        ...
        <circle cx="0" cy="0" r="4.5" fill="${B.flowerGold}" stroke="${B.flowerGoldEdge}" stroke-width="1.5"/></g>`;
    case 'sprout':
      return `<g>
        <path d="M100 30 C 100 24 100 20 100 14" fill="none" stroke="${B.sproutGreenEdge}" stroke-width="3.5" stroke-linecap="round"/>
        <path d="M100 16 C 94 16 88 12 87 5 C 94 4 100 8 100 16 Z" fill="${B.sproutGreen}" stroke="${B.sproutGreenEdge}" stroke-width="2"/>
        <path d="M100 16 C 106 16 112 12 113 5 C 106 4 100 8 100 16 Z" fill="${B.sproutGreen}" stroke="${B.sproutGreenEdge}" stroke-width="2"/></g>`;
    case 'headphones':
      return `<g>
        <path d="M56 84 Q 100 30 144 84" fill="none" stroke="${B.heart}" stroke-width="8" stroke-linecap="round"/>
        <rect x="46" y="76" width="16" height="26" rx="8" fill="${B.heart}" stroke="${B.heartEdge}" stroke-width="2"/>
        <rect x="138" y="76" width="16" height="26" rx="8" fill="${B.heart}" stroke="${B.heartEdge}" stroke-width="2"/></g>`;
    case 'chef':
      return `<g fill="${B.chefWhite}" stroke="${B.chefWhiteEdge}" stroke-width="2.5" stroke-linejoin="round">
    ...
    case 'sunglasses':
      return `<g>
        ...
        <path d="M61 101 L94 101 Q 94 122 77.5 122 Q 61 122 61 101 Z" fill="${B.shades}" stroke="${INK}" stroke-width="3"/>
        <path d="M106 101 L139 101 Q 139 122 122.5 122 Q 106 122 106 101 Z" fill="${B.shades}" stroke="${INK}" stroke-width="3"/>
        <path d="M66 108 l7.5 -3.5 M111 108 l7.5 -3.5" stroke="${white(0.5)}"
          stroke-width="3" stroke-linecap="round"/></g>`;
    case 'bandaid':
      return `<g transform="translate(127 80) rotate(-25)">
        <rect x="-16" y="-6" width="32" height="12" rx="5.5" fill="${B.bandaid}" stroke="${B.bandaidEdge}" stroke-width="2"/>
        <g fill="${B.bandaidEdge}"><circle cx="-5" cy="-1.5" r="1.3"/><circle cx="0" cy="1.5" r="1.3"/>
        <circle cx="5" cy="-1.5" r="1.3"/></g></g>`;
    case 'scarf':
      return `<g fill="${B.heart}" stroke="${B.heartEdge}" stroke-width="2.5" stroke-linejoin="round">
